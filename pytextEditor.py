...
async def run_subprocess(input=None):
    try:
        s_process = await asyncio.create_subprocess_exec('python', 'text_editorApp/defaultpage.py',stdin=asyncio.subprocess.PIPE,stdout=asyncio.subprocess.PIPE,stderr=asyncio.subprocess.PIPE)
        s_process.stdin.write(input.encode())
        await s_process.stdin.drain()
        s_process.stdin.close()
    except Exception as e:
        print(e)
    finally:
        output, error = s_process.stdout.read(),s_process.stderr.read()
        
        print(f"subprocess output: {str(output)}",f"subprocess error: {str(error)}")
...
def openFile(event=None):
    ...
    if file_path:
        ...
        #open defaultpage.py
        asyncio.run(run_subprocess(input=readtext))
...
#creating subframes
file_frame = Frame(ribbon_frame,name='file',bg='#F2F9FF',height=80)
clipboard_frame = Frame(ribbon_frame,name='clipboard',bg='#F2F9FF',height=80)
font_frame = Frame(ribbon_frame,name='fonts',bg='#F2F9FF',height=80)
paragraph_frame = Frame(ribbon_frame,name='paragraph',bg='#F2F9FF',height=80)
insert_frame = Frame(ribbon_frame,name='insert',bg='#F2F9FF',height=80)
editing_frame = Frame(ribbon_frame,name='edit',bg='#F2F9FF',height=80)
...
page = ScrolledText(pageFrame,
            background='#fff',
            foreground='black',
            height=30,
            width=100,
            font=(selectedFont,
            selectedFontSize.get(),"normal"),
            )
